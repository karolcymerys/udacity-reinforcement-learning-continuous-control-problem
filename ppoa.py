...
class ProximalPolicyOptimizationAlgorithm:
    def __init__(self, env: ReacherEnvironment, device: str = 'cpu') -> None:
        self.env = env
        self.device = device
        self.actor_network = PPOActorNetwork(env.state_size(), env.action_size()).to(device)
        self.critic_network = PPOCriticNetwork(env.state_size()).to(device)

    def act(self,
            states: Union[torch.FloatType, torch.cuda.FloatTensor]) -> Tuple[
    ...
    def train(self,
        ...
        def surrogate_fn(
                ...
                _entropy: [torch.FloatType, torch.cuda.FloatTensor],
                _eps: float,
                _beta: float
        ) -> Union[torch.FloatType, torch.cuda.FloatTensor]:
        ...
        scores = []
        actor_optimizer = Adam(self.actor_network.parameters(), actor_network_lr)
        critic_optimizer = Adam(self.critic_network.parameters(), critic_network_lr)
        critic_loss = torch.nn.MSELoss()

        with (tqdm(range(1, max_episodes + 1)) as episodes):
            for episode in episodes:
                trajectories, total_rewards = self.__collect_trajectories(500, gamma, 0.95)
                ...
                for epoch in range(k):
                    ...
                    for start_idx in range(0, len(sample_indices), BATCH_SIZE):
                        ...
                        actions, new_policy_probs, entropy = self.actor_network(states)

                        actor_optimizer.zero_grad()
                        loss = surrogate_fn(new_policy_probs, old_policy_probs, advantages, entropy, eps, beta)
                        loss.backward()
    ...
    def __sample_env(self,
        ...
        for t_i in range(max_t):
            actions, values, log_probs = self.act(previous_response.states)
