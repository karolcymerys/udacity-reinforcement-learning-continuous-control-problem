...
from model import PPOActorNetwork
...
class ProximalPolicyOptimizationAlgorithm:
    ...
    def train(self,
        ...
        scores = []
        actor_optimizer = Adam(self.actor_network.parameters(), actor_network_lr, eps=1e-5)
        ...
        with (tqdm(range(1, max_episodes + 1)) as episodes):
            for episode in episodes:
                ...
                for epoch in range(k):
                    ...
                    for start_idx in range(0, len(sample_indices) // 2, BATCH_SIZE):
                        ...
                        received_v = self.actor_network.critic(states)
    ...
    def __sample_env(self,
        ...
        with torch.no_grad():
            ...
            for t_i in range(max_t):
                actions, log_probs, _ = self.actor_network(previous_response.states)
                response = self.env.step(actions)
                values = self.actor_network.critic(response.states)
